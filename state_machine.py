...
New in this version: CLIMBING_PAUSED state (zone intrusion / unexpected
body in camera frame). Motor holds. Auto-exits to CLIMBING after
ZONE_PAUSE_TIMEOUT_S if zone clears.
"""
...
class State(Enum):
    ...
    WATCH_ME        = auto()
    UP              = auto()
    CLIMBING_PAUSED = auto()   # zone intrusion hold — NEW
    ESTOP           = auto()
...
class Inputs:
    ...
    tension_N: float = 0.0
    cv_clip: bool    = False
    cv_zone: bool    = False   # NEW — unexpected body in camera zone
    estop: bool      = False
    time_s: float    = 0.0
    dt: float        = 0.02
