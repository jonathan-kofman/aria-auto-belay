...
void uart_tx() {
    ...
    snprintf(b,sizeof(b),"S:%d:%.1f:%.2f:%d\n",
             (int)g_state,g_tension_n,g_ropePos,(int)g_motorMode);
    ESP32Serial.print(b);
}
...
void uart_parse(const char* p) {
    ...
    // Firebase → ESP32 → STM32 commands: CMD:PAUSE, CMD:RESUME, CMD:LOCKOUT, CMD:RETURN, CMD:CALIBRATE
        ...
        // CV data: C:<clip_conf>:<height>:<detected>
    ...
    // PID tuner support — added for aria_pid_tuner.py
        ...
        // Future: add g_tension_n_setpoint override
    }
}
...
void handleIdle(){
    motorHold();
    if(g_cvDetected && g_tension_n>T_GROUND) setState(STATE_CLIMBING);
}

void handleClimbing(float dt){
    motorTension(T_BASELINE,dt);
    if(g_tension_n<T_GROUND && !g_cvDetected){setState(STATE_IDLE);return;}
...
void handleTake(){
    if(g_takePend){
        if((millis()-g_takeMs)<=TAKE_CONF_MS){
            if(g_tension_n>=T_TAKE){g_takePend=false;motorRetract(SPD_RETRACT);return;}
        } else {
    ...
    if(g_ropeSpeed>0.1f&&g_tension_n<T_TAKE){setState(STATE_CLIMBING);return;}
...
void handleLower(){
    // Rope snag: tension above take threshold — hold, do not continue paying out
    if(g_tension_n>T_TAKE){ motorHold(); return; }
    motorPayout(SPD_LOWER);
    if(g_tension_n<T_GROUND) setState(STATE_IDLE);
}
...
void loop(){
    ...
    if((now_us-g_lastCtrl)>=(1000000UL/CTRL_HZ)){
        ...
        if(g_ropeSpeed>SPD_FALL && g_tension_n>T_FALL){
